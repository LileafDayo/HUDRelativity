...
public class BetaText : MonoBehaviour
{
    ...
    private DopplerEffectVolume dopplerVolume;

    // Start is called once before the first execution of Update after the MonoBehaviour is created
    void Start()
    {
        var volume = VolumeManager.instance.stack.GetComponent<DopplerEffectVolume>();
        if (volume != null) dopplerVolume = volume;
    }
    ...
    void Update()
    {
       verticalInput = Input.GetAxis("Vertical"); // arrowkeys pressed?
        ...
        if (dopplerVolume != null)
            dopplerVolume.beta.value = beta; // Updates the volume
